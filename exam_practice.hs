...
describePlaceTemperatures xs = [(i, describeTemperature j) | (i, j) <- xs]
...
hotter _ [] = []
hotter temp (x:xs) = if temp < snd x then fst x: hotter temp xs else hotter temp xs
...
insert num (Node a Null) = Node a (Node num Null)
insert num (Node atNum node) = if num > atNum then Node atNum (insert num node) else Node num (Node atNum node)
